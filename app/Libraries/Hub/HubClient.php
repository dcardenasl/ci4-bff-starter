 ...
 *  - Register this BFF's permissions via POST `Config\Hub::$permissionsPath`
 *
...
class HubClient extends AbstractServiceClient
{
     ...
     * BFF never propagates introspect errors as exceptions; an unauthenticated
     * request is the correct user-facing outcome.
     */
    ...
    /**
     * Return a valid service token for this BFF, refreshing if near expiry.
     *
     ...
     * (typically the request's own `Authorization` header, since the hub
     * enforces `users.read` on this endpoint).
     *
